...
<script language="javascript"> 
   ...
   function validate() {
       ...
       if($("input[name=check]:checked").val() == "2"){
           $("input:text[name=deposit_capi]").removeAttr("disabled");
       }
       
       if(isNull(deposit_capi.value)){ deposit_capi.value = "0"; }
   }
</script>
...
$(document).ready(function(){
    ...
    $("input:radio[name=check]").click(function(){
        ...
        if($("input[name=check]:checked").val() == "1"){
            $("input:text[name=deposit_capi]").attr("disabled",false);
            // radio 버튼의 value 값이 1이라면 활성화
 
        }else if($("input[name=check]:checked").val() == "2"){
              $("input:text[name=deposit_capi]").attr("disabled",true);
...
<body>
	<h3 style="margin: 5px 0px;">매입신청</h3>
	<form action='<c:url value="/purchaseFund"/>' onsubmit="return validate();" method="post">
		<table border="1">
			<tr>
			...
			<tr>
				<td colspan="2"><input type="submit" value="신청" id="submit"
